 ...
 async getTeamMembers(leaderId: string) {
  try {
    const result = await pool.query(
      `SELECT 
        ...
        COALESCE(MAX(p.accumulated_points), 0) as total_points  -- ✅ CORRIGIDO
  ...
  async addTeamMember(leaderId: string, data: { email: string; name: string }) {
    ...
    try {
      ...
      // ✅ LOG 3: Antes de criar hierarquia
  ...
  try {
    const result = await pool.query(
      `SELECT 
        ...
        COALESCE(SUM(p_agg.total_points), 0) as total_team_points  -- ✅ CORRIGIDO
       FROM user_hierarchy uh
