...
class PointsService {
    ...
    async getUserPointsHistory(userId) {
        const result = await database_1.pool.query(`SELECT p.*, s.client_name, s.value, s.kilowatts
       FROM points p
    ...
    async getUserTotalPoints(userId) {
        const result = await database_1.pool.query(`SELECT COALESCE(MAX(accumulated_points), 0) as total_points
       FROM points
    ...
    async getUserProgress(userId) {
        ...
        const levelResult = await database_1.pool.query(`SELECT 
       ...
       LIMIT 1`, [totalPoints]);
        if (levelResult.rows.length === 0) {
            ...
            const firstLevel = await database_1.pool.query('SELECT * FROM levels ORDER BY phase_number ASC LIMIT 1');
            return {
                current_level: null,
