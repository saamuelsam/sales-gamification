...
class AdminService {
    ...
    async updateUserStatus(userId, isActive, adminId) {
        ...
        const userInfo = await database_1.pool.query('SELECT name, email FROM users WHERE id = $1', [userId]);
        ...
        // Atualizar status
        await database_1.pool.query(`UPDATE users SET is_active = $1 WHERE id = $2`, [isActive, userId]);
        // ✅ REGISTRAR LOG DE ATIVIDADE
        await (0, activityLogger_1.logActivity)(adminId, activityLogger_1.ActivityAction.UPDATE_USER_STATUS, {
            userId,
            userName,
            userEmail,
    ...
    async updateUserRole(userId, role, adminId) {
        // Buscar informações do usuário antes de atualizar
        const userInfo = await database_1.pool.query('SELECT name, email, role as old_role FROM users WHERE id = $1', [userId]);
        if (userInfo.rows.length === 0) {
            throw new Error('Usuário não encontrado');
        }
        const userName = userInfo.rows[0].name;
        ...
        // Atualizar role
        await database_1.pool.query(`UPDATE users SET role = $1 WHERE id = $2`, [role, userId]);
        // ✅ REGISTRAR LOG DE ATIVIDADE
        await (0, activityLogger_1.logActivity)(adminId, activityLogger_1.ActivityAction.UPDATE_USER_ROLE, {
            userId,
            userName,
            userEmail,
            oldRole,
     ...
     */
    async getAllCommissions(status, search) {
        let query = `
      SELECT nc.id, 
    ...
    `;
        const params = [];
        if (status === 'paid')
            query += ' AND nc.paid = TRUE';
        if (status === 'unpaid')
            query += ' AND nc.paid = FALSE';
        if (search) {
     ...
     */
    async markCommissionAsPaid(id, adminId) {
        // Buscar informações da comissão
        const commissionInfo = await database_1.pool.query(`SELECT nc.commission_amount, l.name as leader_name, m.name as member_name
       FROM network_commissions nc
       JOIN users l ON l.id = nc.leader_id
       JOIN users m ON m.id = nc.team_member_id
       WHERE nc.id = $1`, [id]);
        ...
        // Atualizar comissão
        await database_1.pool.query(`UPDATE network_commissions SET paid = TRUE, paid_at = NOW() WHERE id = $1`, [id]);
        // ✅ REGISTRAR LOG DE ATIVIDADE
        await (0, activityLogger_1.logActivity)(adminId, activityLogger_1.ActivityAction.ADMIN_MARK_COMMISSION_PAID, {
            commissionId: id,
            id,
    ...
    async exportCommissionsCSV() {
        ...
        rows.forEach((r) => {
            csvRows.push(`${r.leader_name},${r.team_member_name},${r.commission_amount},"${r.paid ? 'Paga' : 'Pendente'}",${r.created_at}`);
        });
        return csvRows.join('\n');
    }
    /**
    ...
    async getReports() {
        ...
        `)).rows;
            // 2. Top líder
            const topLeaderQuery = await database_1.pool.query(`
      ...
      `);
            ...
            const salesPerMonthQuery = await database_1.pool.query(`
        SELECT 
      ...
      `);
            ...
            const commissionsSummaryQuery = await database_1.pool.query(`
        SELECT 
      ...
      `);
            ...
            const rolesDistributionQuery = await database_1.pool.query(`
        SELECT 
          role AS name, 
          COUNT(*) AS count,
      ...
      `);
            // 6. Vendas por status
            const salesByStatusQuery = await database_1.pool.query(`
        SELECT 
          status AS name,
          COUNT(*) AS count,
      ...
      `);
            // 7. Top 5 vendedores
            const topSellersQuery = await database_1.pool.query(`
        SELECT 
          u.id,
          u.name,
          u.email,
      ...
      `);
            ...
            const totalPointsQuery = await database_1.pool.query(`
        SELECT 
      ...
      `);
            ...
            const userGrowthQuery = await database_1.pool.query(`
        SELECT 
          TO_CHAR(DATE_TRUNC('month', created_at), 'Mon YYYY') AS month,
          COUNT(*) AS new_users
        FROM users
      ...
      `);
            ...
            const topTeamsQuery = await database_1.pool.query(`
        SELECT 
          l.id,
          l.name AS leader_name,
    ...
    async getSystemConfig() {
        const result = await database_1.pool.query('SELECT key, value FROM system_settings');
        const config = {};
     ...
     */
    async updateSystemConfig(data, adminId) {
        const changedKeys = [];
        for (const key in data) {
            const value = typeof data[key] === 'boolean' ? data[key].toString() : data[key].toString();
            await database_1.pool.query(`INSERT INTO system_settings (key, value, updated_at)
         VALUES ($1, $2, NOW())
         ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`, [key, value]);
            changedKeys.push(key);
        }
        // ✅ REGISTRAR LOG DE ATIVIDADE
        await (0, activityLogger_1.logActivity)(adminId || 'SYSTEM', activityLogger_1.ActivityAction.UPDATE_SYSTEM_CONFIG, {
            changedKeys,
            newValues: data,
     ...
     */
    async createGlobalNotification(title, message, type, target, adminId) {
    ...
    `;
        ...
        const result = await database_1.pool.query(usersQuery);
        ...
        for (const user of users) {
            try {
                await database_1.pool.query(`INSERT INTO notifications (user_id, title, message, type, is_read, created_at)
           VALUES ($1, $2, $3, $4, FALSE, NOW())`, [user.id, title, message, type]);
                successCount++;
        ...
        // ✅ REGISTRAR LOG DE ATIVIDADE
        try {
            await (0, activityLogger_1.logActivity)(adminId, activityLogger_1.ActivityAction.SEND_GLOBAL_NOTIFICATION, {
                title,
                message,
                type,
                target,
     ...
     */
    async getActivityLogs(search, action) {
        let query = `
      SELECT 
     ...
     */
    async getAccessLogs(search, action) {
        let query = `
      SELECT 
    ...
    async createActivityLog(userId, action, details) {
        await (0, activityLogger_1.logActivity)(userId, action, details);
    }
    /**
    ...
    `, [start, end]);
        ...
        const rows = result.rows.map(r => `${r.id};${r.nome};${r.email};${r.cargo};${r.ativo ? 'Sim' : 'Não'};${r.data_cadastro};${r.patrocinador};${r.vendas};R$ ${parseFloat(r.valor_total).toFixed(2)};R$ ${parseFloat(r.comissoes).toFixed(2)}`).join('\n');
        return headers + rows;
    }
    /**
     * Gerar relatório de metas
     */
    async generateGoalsReport(start, end) {
    ...
    `, [start, end]);
        ...
        const rows = result.rows.map(r => `${r.id};${r.vendedor};${r.email};${r.vendas_realizadas};R$ ${parseFloat(r.valor_total).toFixed(2)};${parseFloat(r.kwp_total).toFixed(2)};${r.primeira_venda || 'N/A'};${r.ultima_venda || 'N/A'}`).join('\n');
        return headers + rows;
    }
}
