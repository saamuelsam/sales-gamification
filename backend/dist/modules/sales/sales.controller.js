...
class SalesController {
        ...
        try {
            ...
            if (sale_type === 'consortium') {
                if (!numericConsortiumValue || !numericConsortiumTerm)
                    return responses_1.ApiResponse.error(res, 'Consórcio requer consortium_value e consortium_term', 400);
            ...
            return responses_1.ApiResponse.error(res, error.message || 'Erro ao registrar venda', 500);
        }
    }
    /**
    ...
    async updateStatus(req, res) {
        try {
            ...
            return responses_1.ApiResponse.error(res, error.message || 'Erro ao atualizar status', 500);
        }
    }
    /**
        ...
        try {
            ...
            if (value !== undefined) {
                ...
                if (!Number.isFinite(val) || val <= 0)
                    return responses_1.ApiResponse.error(res, 'Valor deve ser positivo', 400);
                updateData.value = val;
            }
            if (kilowatts !== undefined) {
                const kw = Number(kilowatts);
                if (!Number.isFinite(kw) || kw <= 0)
                    return responses_1.ApiResponse.error(res, 'Kilowatts deve ser positivo', 400);
            ...
            return responses_1.ApiResponse.error(res, error.message || 'Erro ao atualizar venda', 500);
        }
    }
    /**
    ...
    async listSales(req, res) {
        try {
            ...
            return responses_1.ApiResponse.error(res, error.message || 'Erro ao listar vendas', 500);
        }
    }
    async getSale(req, res) {
        try {
            ...
            return responses_1.ApiResponse.error(res, error.message || 'Erro ao buscar venda', 404);
        }
    }
    ...
    async getStats(req, res) {
        try {
            ...
            return responses_1.ApiResponse.error(res, error.message || 'Erro ao obter estatísticas', 500);
        }
    }
    async getChartData(req, res) {
        try {
            ...
            return responses_1.ApiResponse.error(res, error.message || 'Erro ao buscar dados de gráficos', 500);
        }
    }
    async deleteSale(req, res) {
        ...
        try {
            ...
            const sale = await client.query(`SELECT kilowatts, client_name, value FROM sales WHERE id = $1 AND user_id = $2`, [id, userId]);
            ...
            await client.query(`UPDATE users SET points = GREATEST(0, points - $1) WHERE id = $2`, [pointsToRemove, userId]);
            ...
            await level_service_1.levelService.checkLevelUp(userId, newPoints, client);
            ...
            return responses_1.ApiResponse.success(res, { id, pointsRemoved: pointsToRemove, newPoints }, 'Venda deletada com sucesso');
        }
        catch (error) {
            await client.query('ROLLBACK');
