...
class CommissionService {
    ...
    async processNetworkCommission(memberId, saleValue, points, saleId) {
        try {
            logger_1.logger.info(`📝 [COMISSÃO] Iniciando para membro: ${memberId}`);
            // 1. Buscar o líder direto
            const leaderResult = await database_1.pool.query(`SELECT u.id, u.role, u.name FROM user_hierarchy uh
         JOIN users u ON uh.leader_id = u.id
         WHERE uh.subordinate_id = $1 AND uh.line_level = 1
         LIMIT 1`, [memberId]);
            if (leaderResult.rows.length === 0) {
                logger_1.logger.info(`⚠️ [COMISSÃO] Membro SEM líder`);
                return;
            }
            ...
            if (commissionRate === 0) {
                logger_1.logger.info(`⚠️ [COMISSÃO] Líder tem taxa 0%`);
                return;
            }
            // 2. Calcular comissão
            const commissionAmount = (saleValue * commissionRate) / 100;
            logger_1.logger.info(`💰 [COMISSÃO] Valor: R$${commissionAmount.toFixed(2)}`);
            // 3. Buscar informações do membro
            const memberInfo = await database_1.pool.query('SELECT name, email FROM users WHERE id = $1', [memberId]);
            ...
            logger_1.logger.info(`💾 [COMISSÃO] Inserindo no banco...`);
            const insertResult = await database_1.pool.query(`INSERT INTO network_commissions 
         ...
         RETURNING id, commission_amount`, [
            ...
            logger_1.logger.info(`✅ [COMISSÃO] Inserida com sucesso!`);
            logger_1.logger.info(`✅ [COMISSÃO] ID: ${commissionId}`);
            logger_1.logger.info(`✅ [COMISSÃO] Valor: R$${insertResult.rows[0].commission_amount}`);
            // ✅ REGISTRAR LOG DE ATIVIDADE - NOVA COMISSÃO
            await (0, activityLogger_1.logActivity)(leaderId, activityLogger_1.ActivityAction.NEW_COMMISSION, {
                commissionId,
                memberId,
                memberName,
                memberEmail,
            ...
            logger_1.logger.error(`❌ [COMISSÃO] ERRO: ${error.message}`);
            logger_1.logger.error(`❌ [COMISSÃO] Stack: ${error.stack}`);
        }
    }
    /**
    ...
    async getNetworkCommissions(leaderId) {
        try {
            logger_1.logger.info(`📋 Buscando comissões para: ${leaderId}`);
         ...
         LIMIT 100`, [leaderId]);
            logger_1.logger.info(`✅ Encontradas ${result.rows.length} comissões`);
            return result.rows;
        }
        catch (error) {
            logger_1.logger.error(`❌ Erro ao buscar: ${error.message}`);
            return [];
        }
    }
    /**
     * ✅ Resumo de comissões
     */
    async getNetworkCommissionsSummary(leaderId) {
        try {
            const result = await database_1.pool.query(`SELECT 
         ...
         WHERE leader_id = $1`, [leaderId]);
            ...
            logger_1.logger.error(`❌ Erro ao buscar resumo: ${error.message}`);
            return {
                total_commissions: 0,
     ...
     */
    async getMonthlyNetworkCommissions(leaderId) {
        try {
        ...
        `, [leaderId]);
            ...
            logger_1.logger.info(`✅ Encontrados ${formattedData.length} meses de comissões`);
            return formattedData;
        }
        catch (error) {
            logger_1.logger.error(`❌ Erro ao buscar comissões mensais: ${error.message}`);
     ...
     * ✅ Resumo completo
     */
    async getCompleteCommissionsSummary(userId) {
        try {
            ...
            logger_1.logger.error(`❌ Erro: ${error.message}`);
            return {
                network: null,
                total_earned: 0,
                total_paid: 0,
     ...
     */
    async markNetworkCommissionAsPaid(commissionId, leaderId) {
        try {
            // Buscar informações da comissão antes de atualizar
            const commissionInfo = await database_1.pool.query(`SELECT nc.commission_amount, u.name as member_name
         FROM network_commissions nc
         JOIN users u ON nc.team_member_id = u.id
         WHERE nc.id = $1 AND nc.leader_id = $2`, [commissionId, leaderId]);
            ...
            // Atualizar comissão
            const result = await database_1.pool.query(`UPDATE network_commissions 
         SET paid = TRUE, paid_at = CURRENT_TIMESTAMP
         WHERE id = $1 AND leader_id = $2
         RETURNING *`, [commissionId, leaderId]);
            if (result.rows.length === 0) {
                throw new Error('Erro ao atualizar comissão');
            }
            logger_1.logger.info(`✅ Comissão ${commissionId} marcada como paga`);
            // ✅ REGISTRAR LOG DE ATIVIDADE - COMISSÃO PAGA
            await (0, activityLogger_1.logActivity)(leaderId, activityLogger_1.ActivityAction.MARK_COMMISSION_PAID, {
                commissionId,
            ...
            logger_1.logger.error(`❌ Erro ao marcar como paga: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async getConsolidatedCommissionsReport() {
        ...
        ORDER BY network_total DESC`);
            logger_1.logger.info(`📈 Relatório: ${result.rows.length} usuários com comissões`);
            return result.rows;
        }
        catch (error) {
            logger_1.logger.error(`❌ Erro ao gerar relatório: ${error.message}`);
            return [];
        }
    }
    /**
    ...
    async exportCommissionsCSV() {
        try {
            ...
            logger_1.logger.info(`📥 CSV exportado: ${rows.length} linhas`);
            return { headers, rows };
        }
        catch (error) {
            logger_1.logger.error(`❌ Erro ao exportar: ${error.message}`);
            throw error;
        }
    }
    /**
