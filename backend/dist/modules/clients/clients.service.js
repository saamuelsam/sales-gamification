...
class ClientsService {
    async createClient(userId, data) {
        const result = await database_1.pool.query(`INSERT INTO clients (
    ...
    async listUserClients(userId) {
        const result = await database_1.pool.query('SELECT * FROM clients WHERE user_id = $1 ORDER BY name ASC', [userId]);
        return result.rows;
    }
    async getClientById(clientId, userId) {
        const result = await database_1.pool.query('SELECT * FROM clients WHERE id = $1 AND user_id = $2', [clientId, userId]);
    ...
    // --- MÉTODO ADICIONADO ---
    async updateClient(clientId, userId, data) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
